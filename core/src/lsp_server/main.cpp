...
try {
  ...
  for (;;) {
    ...
    if (method == "initialize") {
      // initializationOptions: { stdlibPath?: string }
      try {
        if (
          params.contains("initializationOptions") && params["initializationOptions"].is_object()) {
      ...
      // Capabilities
      const std::vector<std::string> semantic_types = {
        "keyword", "class", "function", "variable", "parameter", "property", "type", "decorator"};
      const std::vector<std::string> semantic_mods = {
        "declaration", "modification", "defaultLibrary"};
      ...
      caps["semanticTokensProvider"] = json{
        {"legend", json{{"tokenTypes", semantic_types}, {"tokenModifiers", semantic_mods}}},
        {"full", true}};
      ...
      try {
        ...
        try {
          ...
          if (it != j.end() && it->is_array()) {
            ...
            const std::string_view doc_text = doc_text_opt ? *doc_text_opt : std::string_view(text);
        ...
        diag_note["diagnostics"] = std::move(diag_items);
    ...
    if (method == "textDocument/didChange") {
      ...
      try {
        ...
        try {
          ...
          const auto it = j.find("items");
          if (it != j.end() && it->is_array()) {
            const std::string_view doc_text = st.docs[uri];
            for (const auto & item : *it) {
              if (!item.is_object()) continue;
              const auto msg_it = item.find("message");
              const auto sev_it = item.find("severity");
              ...
              d["message"] = msg_it->get<std::string>();
              d["severity"] = diag_severity_to_lsp(sev_it->get<std::string>());
              d["source"] = item.value("source", "bt-dsl");
              if (item.contains("code") && item["code"].is_string()) {
                d["code"] = item["code"].get<std::string>();
              }
    ...
    if (method == "textDocument/hover") {
      ...
      try {
        ...
        try {
          ...
          hover["contents"] = json{
            {"kind", "markdown"},
            {"value", j["contents"].get<std::string>()},
          };
    ...
    if (method == "textDocument/semanticTokens/full") {
      ...
      try {
        ...
        try {
          ...
          if (it != j.end() && it->is_array()) {
            for (const auto & t : *it) {
              ...
              const auto ti = std::find(semantic_types.begin(), semantic_types.end(), type);
              if (ti == semantic_types.end()) continue;

              Tok tok;
              tok.start = range.start;
              tok.end = range.end;
              tok.type_idx = static_cast<int>(std::distance(semantic_types.begin(), ti));
              tok.mod_bits = 0;
              ...
              if (t.contains("modifiers") && t["modifiers"].is_array()) {
                for (const auto & mv : t["modifiers"]) {
                  ...
                  const auto mi = std::find(semantic_mods.begin(), semantic_mods.end(), mod);
                  if (mi != semantic_mods.end()) {
                    const int bit = static_cast<int>(std::distance(semantic_mods.begin(), mi));
                    tok.mod_bits |= 1 << bit;
                  }
                }
              }
        ...
        for (const auto & t : toks) {
          ...
          const int delta_start =
            (first || delta_line != 0) ? start_char : (start_char - prev_char);
