...
TEST(AstProgram, TopLevel)
{
  ...
  ASSERT_EQ(p->extern_types().size(), 1U);
  EXPECT_EQ(p->extern_types()[0]->name, "Pose");
  EXPECT_EQ(p->extern_types()[0]->docs.size(), 1U);

  ASSERT_EQ(p->type_aliases().size(), 1U);
  EXPECT_EQ(p->type_aliases()[0]->name, "PoseVec");
  ASSERT_NE(p->type_aliases()[0]->aliasedType, nullptr);

  ASSERT_EQ(p->global_vars().size(), 1U);
  EXPECT_EQ(p->global_vars()[0]->name, "g");
  ASSERT_NE(p->global_vars()[0]->type, nullptr);
  ASSERT_NE(p->global_vars()[0]->initialValue, nullptr);

  ASSERT_EQ(p->global_consts().size(), 1U);
  EXPECT_EQ(p->global_consts()[0]->name, "C");
  ASSERT_NE(p->global_consts()[0]->value, nullptr);
