...
- コンパイル時評価において、数値演算のオーバーフローや精度損失が発生する場合は **コンパイルエラー**
  とします。

> [!NOTE]
> 実行時における評価順序・オーバーフロー/丸め等の詳細は
> **規定しません**。これらは処理系および実行環境に依存します（implementation-defined）。
