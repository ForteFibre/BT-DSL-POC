...
> [!NOTE] 「将来の予約語」は現時点では別途規定しません（TBD）。
...
加えて、次の数値エスケープをサポートしてよいものとします（推奨）。

| 表記 | 意味 |
| :--- | :--- |
| `\\xHH` | 1 バイト（16 進 2 桁） |
| `\\uHHHH` | Unicode スカラー値（16 進 4 桁） |
| `\\u{H...}` | Unicode スカラー値（16 進 1 桁以上、`}` まで） |

> [!IMPORTANT]
> `string` は UTF-8 文字列です。従って、エスケープ解釈後の文字列値は**妥当な UTF-8**でなければなりません。
> `\\u...` は UTF-8 に符号化されます。`\\xHH` を提供する処理系は、結果が不正な UTF-8 となる場合に診断を出してよいものとします。
