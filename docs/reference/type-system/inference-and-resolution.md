...
1. **初期型の付与**
   - 型注釈なしの `var` / `const`
     宣言は、初期値があればその式型から開始し、初期値がなければ未解決型変数 `?` から開始します。
...
4. **デフォルト適用（Defaulting）**
   - スコープ終了までに `{integer}` / `{float}`
     が解決できない場合、デフォルト型へ解決します（それぞれ `int32` / `float64`）。
...
- `const` は初期化必須です。
- 初期値は `const_expr` でなければなりません（構文は
  [構文](../syntax.md#_2-7-定数式constant-expressions)）。
