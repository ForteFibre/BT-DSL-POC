...
ここでの $N$ は **非負の整数**であり、コンパイル時に定数として決定できなければなりません。
構文上は `string<SIZE>` のように識別子を用いてもよく、この場合その識別子は「コンパイル時定数として評価可能な整数」を参照しなければなりません。
...
> [!IMPORTANT]
> `[T; <=N]` は（`vec<T>` と同様に）実行時に「現在長（length）」を持つものとして扱われますが、
> その length を取得する式や API は、現状 **未規定（TBD）** です。
...
- 外部型の互換性は「名前解決（import を含む）後に **同一の `extern type` 宣言**を指すもの同士」に限定されます（詳細は 3.3）。

> [!NOTE]
> 異なるモジュールがそれぞれ `extern type Pose;` を定義している場合、`Pose` が参照位置で一意に解決できないなら、その参照は曖昧性によりコンパイルエラーです（宣言とスコープ 4.1.3/4.2.2）。
...
> [!NOTE]
> 型構文のワイルドカード `_` / `_?` は「その位置の型を推論せよ」という要求です。
> 処理系は、`_` が現れた位置に対して新しい未解決型変数 `?` を導入し、`_?` が現れた位置に対しては
> 「Nullable であることが確定した」未解決型変数（結果として必ず `T?` 形に解決されるもの）を導入する、と考えて差し支えありません。
