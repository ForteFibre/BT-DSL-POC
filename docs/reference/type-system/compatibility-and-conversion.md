...
本ドキュメントは、型 $S$ の値を型 $T$
として扱えるか（代入・引数渡しの可否）と、型変換の規則を定義します。
...
互換性は「どの位置でその値を要求するか」により規則が異なります。
本章では、少なくとも次の文脈を区別します。

- **値文脈（Value Compatibility）**: 代入および `in` 引数のように「値として渡す」文脈。
  - 暗黙変換（3.3.3）および `as`（3.3.4）を適用できます。
- **参照文脈（Reference Compatibility）**: `ref` / `mut` 引数のように「参照として渡す」文脈。
  - 正規化後に完全一致が必要です（3.3.7）。
- **出力文脈（Output Compatibility）**: `out` 引数のように「書き込み先として渡す」文脈。
  - 追加特例（Output Widening 等）を含みます（3.3.3, 3.3.7）。
...
> [!NOTE]
> 縮小変換の結果（オーバーフロー、丸めなど）の詳細な数値意味論は
> **規定しません**。これは処理系および実行環境に依存します（environment/implementation-defined）。処理系は少なくとも「情報落ちが起こり得る」ことを前提に、静的解析や警告を行っても構いません。
...
- 静的配列を `vec<T>` として扱うには、明示的キャストが必要です（例:
  `[1,2,3] as vec<_>`）。
...
- 型は完全一致が基本です。
- ただし **Output Widening**
  により、出力側が小さい数値型で受け手が大きい数値型である場合に限り、暗黙に許容されます。

> [!NOTE]
> Nullable と `out` の接続に関する追加規則（例: `T?` を `out T` に渡す）については、
> 静的解析と安全性 6.2.3 を参照してください。

---

## 3.3.8 外部型（Opaque Type）の互換性

- 外部型は、名前解決（import を含む）後に **同一の `extern type` 宣言**を指す場合に限り互換です。
