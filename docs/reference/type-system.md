...
- **型構文**: `[T; N]`
- **メモリ配置**: スタック（ローカル Blackboard）または静的領域（グローバル Blackboard）。ヒープ割り当ては発生しません。
...
> [!IMPORTANT]
> ローカル定数もグローバル定数と同様に、コンパイル時に値が確定している必要があります。実行時の値（Blackboard やパラメータ）を使用することはできません。
...
1. **制約の収集**:
   - Blackboard 宣言 `var x` は初期状態 `?` または `{integer}`/`{float}` で始まります。
...
### 3.3 ローカル Blackboard の型決定

ローカル Blackboard の型は以下の優先順位で決定されます：
...
### 3.4 グローバル Blackboard の型決定

グローバル Blackboard の型は以下の優先順位で決定されます：
...
> [!IMPORTANT]
> グローバル Blackboard の型は、明示的な型注釈または初期値から推論される必要があります。両方がない場合はコンパイルエラーとなります。
...
グローバル定数の型はローカル Blackboard と同様のルールに従います。
...
> [!NOTE] `ref` および `mut` 引数では、値の参照が渡されるため、型が完全に一致しなければなりません。
...
| `[T; N]`        | `vec&lt;T&gt;`     | Error | 暗黙変換不可（`as vec<_>` 必須）     |
| `vec&lt;T&gt;`  | `[T; N]`           | Error | 縮小変換不可                         |
| `vec&lt;T&gt;`  | `vec&lt;T&gt;`     | OK    | 参照コピーまたはクローン（実装依存） |

> [!IMPORTANT]
> 配列型間の変換にはコストがかかるため、暗黙の変換は禁止されています。静的配列を動的配列に変換する場合は、明示的なキャスト（`as vec<_>`）が必要です。
...
// 2. 静的 -> 動的 (暗黙変換は禁止)
ProcessDynamic(static_data); // Error: 暗黙変換は不可
ProcessDynamic([1.0, 2.0]);  // Error: 暗黙変換は不可

// 3. 静的 -> 動的 (明示的キャストが必要)
ProcessDynamic(static_data as vec<_>);  // OK: 明示的キャスト
ProcessDynamic([1.0, 2.0] as vec<_>);   // OK: 明示的キャスト

// 4. サイズ違い
ProcessStatic([1.0, 2.0]);   // Error: [float64; 2] は [float64; 4] に適合しない
```

### 5.5 暗黙の拡大 (Implicit Widening for Output)
...
動的配列を使用するには、`vec!` マクロを使用するか、明示的なキャストを行います。

```bt-dsl
// vec! マクロを使用（推奨）
var list = vec![1, 2, 3];              // vec<int32>
var list: vec<int32> = vec![1, 2, 3];  // vec<int32>（型注釈あり）
var v = vec![0; 10];                   // vec<int32> (サイズ10、初期値0)

// 明示的キャストによる変換
var list: vec<int32> = [1, 2, 3] as vec<_>;  // 静的配列から動的配列への変換
```

> [!IMPORTANT] 静的配列リテラルから動的配列への暗黙変換は禁止されています。動的配列が必要な場合は
> `vec!` マクロを使用するか、明示的なキャスト（`as vec<_>`）を行ってください。
