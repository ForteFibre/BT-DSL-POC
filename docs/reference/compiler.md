...
コンパイラは処理を以下の2段階に分離します：

| フェーズ | 対象機能 | 説明 |
|---------|---------|------|
| **フロントエンド** | AST 構築、型推論、型検査 | 全ての BT-DSL 言語機能を完全にサポート |
| **ターゲット検証** | ターゲット制約チェック | 指定ターゲットでサポートされない機能の使用を検出しエラーを報告 |

- `btc check`: フロントエンド処理 + ターゲット検証を実行（コード生成なし）
- `btc build`: フロントエンド処理 + ターゲット検証 + コード生成を実行
